...
impl<'a, Message: Clone + 'static> Section<'a, Message> {
    ...
    /// Add a generic child element to the section's list column.
    ...
    /// Add a generic child element to the section's list column, if `Some`.
    ...
    /// Add a button child element to the section's list column.
    pub fn add_button(
        mut self,
        item: impl Into<Element<'a, Message>>,
        on_press: impl Into<Option<Message>>,
    ) -> Self {
        self.children = self.children.add_button(item.into(), on_press);
        self
    }

    /// Add a button child element to the section's list column, if `Some`.
    pub fn add_button_maybe(
        self,
        item: Option<impl Into<Element<'a, Message>>>,
        on_press: impl Into<Option<Message>>,
    ) -> Self {
        if let Some(item) = item {
            self.add_button(item, on_press)
        } else {
            self
        }
    }
