...
{
    #[setters(skip)]
    on_selected: Arc<dyn Fn(window::Id, usize) -> Message + Send + Sync>,
    #[setters(skip)]
    selections: &'a [S],
    #[setters]
    icons: &'a [icon::Handle],
    ...
    #[setters(skip)]
    on_surface_action: Option<Box<dyn Fn(surface::Action) -> Message + 'a>>,
    #[setters(skip)]
    ...
    [S]: std::borrow::ToOwned,
{
    ...
    pub fn new(
        selections: &'a [S],
        selected: Option<usize>,
        on_selected: impl Fn(window::Id, usize) -> Message + 'static + Send + Sync,
    ) -> Self {
        Self {
    ...
    pub fn with_popup(
        mut self,
        parent_id: window::Id,
        on_surface_action: impl Fn(surface::Action) -> Message + 'a,
        action_map: impl Fn(Message) -> AppMessage + 'static + Send + Sync,
    ) -> Self {
        self.window_id = Some(parent_id);
        self.on_surface_action = Some(Box::new(on_surface_action));
    ...
    [S]: std::borrow::ToOwned,
{
    ...
    fn on_event(
        ...
        update::<S, Message, AppMessage>(
            ...
            self.window_id,
            self.on_surface_action.as_deref(),
...
>(
    ...
    #[cfg(all(feature = "winit", feature = "wayland"))]
    positioner: iced_runtime::platform_specific::wayland::popup::SctkPositioner,
    on_selected: Arc<dyn Fn(window::Id, usize) -> Message + Send + Sync + 'static>,
    selected: Option<usize>,
    selections: &[S],
    state: impl FnOnce() -> &'a mut State,
    _window_id: Option<window::Id>,
    on_surface_action: Option<&'a dyn Fn(surface::Action) -> Message>,
        ...
        | Event::Touch(touch::Event::FingerPressed { .. }) => {
            ...
            if is_open {
                ...
                *hovered_guard = selected;
                ...
                state.popup_id = id;
                #[cfg(all(feature = "winit", feature = "wayland"))]
                if let Some(((on_open, parent), action_map)) =
                        ...
                        Some(Box::new(move || {
                            ...
                            let e: Element<'static, crate::Action<AppMessage>> =
                                Element::from(menu_widget(
                                    ...
                                    icons.clone(),
                                    selected_option,
                                    Arc::new(move |i| on_selected.clone()(id, i)),
                                ))
                                .map(move |m| crate::Action::App(action_map.clone()(m)));
                            e
                        })),
                    );
                    shell.publish(on_open(get_popup_action));
                }
                event::Status::Captured
            } else {
        ...
        Event::Mouse(mouse::Event::WheelScrolled {
            ...
            if state.keyboard_modifiers.command() && cursor.is_over(layout.bounds()) && !is_open {
                ...
                if selections.len() < next_index {
                    shell.publish((on_selected)(window::Id::NONE, next_index));
                }
...
pub fn overlay<'a, S: AsRef<str> + Send + Sync + Clone + 'static, Message: std::clone::Clone + 'a>(
    ...
    icons: &'a [icon::Handle],
    selected_option: Option<usize>,
    on_selected: &'a dyn Fn(window::Id, usize) -> Message,
    translation: Vector,
    ...
    if state.is_open.load(Ordering::Relaxed) {
        ...
        let menu = Menu::new(
            ...
            |option| {
                ...
                (on_selected)(window::Id::NONE, option)
            },
            None,
        )
        .width({
